...
def get_avg_task_df(all_tasks_df, all_tasks, task, metrics, immune_task_indices, metabolic_task_indices, tumor_task_indices):
...
if __name__ == "__main__":
    ...
    # Go through all the different subtypes and create the plots
    for subtype in SUBTYPES_TABLE_DICT.keys():

        # Create output directory if it doesn't exist and add subtype to the output path
        output_path = os.path.join(args.output_path, subtype)
        ...
        for task in all_tasks:
            ...
            if task not in ('Average Across Tasks', 'Average Across Immune Tasks', 'Average Across Metabolic Tasks', 'Average Across Tumor Tasks'):
                ...
                task_df = get_avg_task_df(all_tasks_df, all_tasks, task, metrics, immune_task_indices, metabolic_task_indices, tumor_task_indices)
            ...
            if task == 'Angiogenesis':
                ...
                metrics_matrix = np.zeros((1000, 10, 1, len(metrics)))
                for t_id, t_n in enumerate(all_tasks):
                    if t_n in ('Average Across Tasks', 'Average Across Immune Tasks', 'Average Across Metabolic Tasks', 'Average Across Tumor Tasks'):
                        ...
                        for metric_id, metric_name in metrics_dict.items():
                            ...
                            filtered_data = violin_df_raw[t_mask & metric_mask]
                            filtered_data = filtered_data.values
                            # print(filtered_data.shape)
                            # print(filtered_data)
                            if filtered_data.shape[0] == 1000:
                                metrics_matrix[:, t_id, 0, metric_id] = filtered_data[:, 3]
                ...
                violin_dict_avg['task'].extend(['Average Across Tasks'] * 1 * len(metrics) * 1000)
                ...
                violin_dict_avg_immune['task'].extend(['Average Across Immune Tasks'] * 1 * len(metrics) * 1000)
                ...
                violin_dict_avg_metabolic['task'].extend(['Average Across Metabolic Tasks'] * 1 * len(metrics) * 1000)
                ...
                violin_dict_avg_tumor['task'].extend(['Average Across Tumor Tasks'] * 1 * len(metrics) * 1000)


                # Go through the models
                for m_idx, m_name in enumerate(["H&E WSI-only"]):
